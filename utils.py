...
def _is_hours_like_field(field_name):
    ...
    return (
        normalized_name in explicit_hours_labels
        or '(horas)' in normalized_name
        or ' hora' in normalized_name
        or normalized_name.startswith('tempo em: ')
    )


def get_chart_value_format(chart_config, candidate_fields=None):
    """Formato unificado de valores no chart config (compatível com configs legadas).

    Se não existir configuração explícita, tenta inferir por nomes de campos (ex.: '(horas)').
    """
    explicit_format = chart_config.get('value_format') or chart_config.get('y_axis_format')
    if explicit_format:
    ...
    if any(_is_hours_like_field(field_name) for field_name in candidate_fields):
        return 'hours'
...
def render_chart(chart_config, df, chart_key):
    ...
    try:
        ...
        if chart_type in ['barra', 'barra_horizontal', 'linha_agregada', 'pizza', 'treemap', 'funil', 'tabela']:
            ...
            if get_chart_value_format(chart_config, [y]) == 'hours' and y in plot_df.columns:
            ...
            if chart_config.get('show_data_labels'):
                text_template = '%{text:.2f}h' if get_chart_value_format(chart_config, [y]) == 'hours' else '%{text:.2s}'
            ...
            value_format = get_chart_value_format(chart_config, [chart_config.get('num_field'), chart_config.get('den_field'), chart_config.get('base_field')])
            ...
            if rows and values:
                if values == "Contagem de Issues":
                    ...
                    if aggfunc in ('soma', 'média'):
                        ...
                        if get_chart_value_format(chart_config, [values]) == 'hours':
            ...
            if get_chart_value_format(chart_config, [measure]) == 'hours' and measure_col_for_plotting in df_chart.columns:
            ...
            metric_suffix = "h" if get_chart_value_format(chart_config, [measure]) == 'hours' else ""
            st.metric(
                label=title,
