...
def build_date_aggregation_metadata(plot_df: pd.DataFrame, x_col: str, date_aggregation: str):
    ...
    standard_period_map = {
        'Dia': ('D', {'dtick': 86400000, 'tickformat': '%d/%m/%Y'}),
        'Semana': ('W-SUN', {'dtick': 7 * 24 * 60 * 60 * 1000, 'tickformat': '%d/%m/%Y'}),
        'Mês': ('M', {'dtick': 'M1', 'tickformat': '%m/%Y'}),
        'Trimestre': ('Q', {'dtick': 'M3', 'tickformat': '%m/%Y'}),
        'Ano': ('Y', {'dtick': 'M12', 'tickformat': '%Y'}),
    }

    if date_aggregation in standard_period_map:
        period_code, x_axis_time_settings = standard_period_map[date_aggregation]
        period_series = plot_df[x_col].dt.to_period(period_code)
...
def render_chart(chart_config, df, chart_key):
    ...
    try:
        ...
        if chart_type in ['barra', 'barra_horizontal', 'linha_agregada', 'pizza', 'treemap', 'funil', 'tabela']:
            ...
            if date_aggregation and date_aggregation != 'Nenhum' and x in plot_df.columns:
                ...
                if pd.api.types.is_datetime64_any_dtype(plot_df[x]):
                    ...
                    if period_start_col in plot_df.columns:
                        ...
                        aggregated_x_label_col = period_label_col
            ...
            if aggregated_x_label_col:
                plot_args["custom_data"] = [aggregated_x_label_col]
            ...
            if aggregated_x_label_col:
                fig.update_traces(hovertemplate=f"{date_aggregation}: %{{customdata[0]}}<br>{y}: %{{y}}<extra></extra>")
                if x_axis_time_settings:
                    fig.update_xaxes(type='date', **x_axis_time_settings)
