...
from app.models.schemas import ActivateConnectionRequest, JiraConnectionCreate, JiraConnectionOut, JqlCountRequest, ProjectDataRequest
from app.services.jira_service import activate_connection, get_active_client, get_jira_fields, get_jql_issue_count, get_project_issues, get_projects, issues_to_rows, list_user_connections, save_connection
...
def create_connection(payload: JiraConnectionCreate, user: CurrentUser = Depends(get_current_user)):
    ...
    return {"id": connection["id"], "name": connection["name"], "jira_url": connection["jira_url"], "jira_email": connection["jira_email"], "active": False}
...
def project_data(payload: ProjectDataRequest, user: CurrentUser = Depends(get_current_user)):
    ...
    issues = get_project_issues(client, payload.project_key, payload.jql_filter, payload.standard_fields, payload.custom_fields)
    return {"project_key": payload.project_key, "total": len(issues), "rows": issues_to_rows(issues, fields_catalog), "fields": fields_catalog}
