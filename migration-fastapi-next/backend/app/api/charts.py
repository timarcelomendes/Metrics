...
def save_chart(payload: DashboardChartSaveRequest, user: CurrentUser = Depends(get_current_user)):
    ...
    project_layout = layout.setdefault(
        payload.project_key,
        {
            "active_dashboard_id": payload.dashboard_id,
            "dashboards": {
                payload.dashboard_id: {
                    "id": payload.dashboard_id,
                    "name": "Dashboard Principal",
                    "tabs": {payload.tab_name: []},
                }
            },
        },
    )
    dashboard = project_layout.setdefault("dashboards", {}).setdefault(
        payload.dashboard_id,
        {"id": payload.dashboard_id, "name": "Dashboard Principal", "tabs": {payload.tab_name: []}},
    )
