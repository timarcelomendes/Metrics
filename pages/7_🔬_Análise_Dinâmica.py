                ...
                with st.spinner("Buscando e preparando dados..."):
                    ...
                    for i in issues:
                        ...
                        for field in custom_fields:
                            ...
                            elif value is not None: issue_data[field_name_custom] = str(value)
...
with st.expander("Filtros Globais (afetam a pré-visualização)", expanded=False):
    ...
    tipos_selecionados = filter_cols[0].multiselect("Filtrar por Tipo", options=tipos, key='filter_type'); responsaveis_selecionados = filter_cols[1].multiselect("Filtrar por Responsável", options=resp, key='filter_resp')
    status_selecionados = filter_cols[2].multiselect("Filtrar por Status", options=stats, key='filter_status'); prioridades_selecionadas = filter_cols[3].multiselect("Filtrar por Prioridade", options=prios, key='filter_prio')
    filtered_df = df.copy()
...
if editing_mode:
    ...
    with col1:
        if st.button("Salvar Alterações", type="primary", use_container_width=True, icon="💾"):
            dashboard_items = load_config(DASHBOARD_LAYOUT_FILE, [])
            ...
            if item_index is not None:
                ...
                dashboard_items[item_index] = new_chart_config; save_config(dashboard_items, DASHBOARD_LAYOUT_FILE)
    ...
    if len(st.session_state.get('dashboard_items', [])) >= 12:
        st.warning("Limite de 12 visualizações no dashboard atingido.")
    else:
        if st.button("Adicionar ao Meu Dashboard", type="primary", use_container_width=True, icon="➕"):
            if chart_config: 
                st.session_state.dashboard_items.append(chart_config)
                save_config(st.session_state.dashboard_items, DASHBOARD_LAYOUT_FILE)
                st.success(f"Visualização '{chart_config.get('title')}' adicionada!")
                st.session_state.pop('chart_to_edit', None)
